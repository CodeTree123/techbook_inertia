...
const AllWorkOrder = ({ w_orders }) => {
  ...
  const fetchWorkOrders = async (search = '', stage = 0) => {
    setLoading(true);
    try {
      const response = await fetch(`/api/all-wo-lists?search=${search}&stage=${stage}`);
      const data = await response.json();
      setWorkOrders(data.w_orders.data); // Assuming pagination is in data
    } catch (error) {
  ...
  useEffect(() => {
    fetchWorkOrders(searchQuery, activeFilter); // Fetch with current search query
                  ...
                  <tr className='rounded-3'>
                    ...
                    <td className='border-0' style={{ borderRadius: '0 10px 10px 0' }}>
                      <div className='d-flex gap-1'>
                        <Link href={`/pdf/work/order/download/${wo.id}`} download  className='btn'><i class="fa-solid fa-download text-primary"></i></Link>
