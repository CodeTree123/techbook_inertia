                                ...
                                @if (@$firstHour->work_order_id == $invoice->id)
                                    <form action="{{ route('admin.invoice.updateFirstHourProduct', $invoice->id) }}"
                                        ...
                                        <tr class="calc-tr">
                                            <td>
                                                <input type="text"
                                                    class="total-hours py-2 px-0 my-input-disable-class bg-white"
                                ...
                                @foreach ($extraHourProducts as $extraHourProduct)
                                    <form action="{{ route('admin.invoice.updateExtraHour', $extraHourProduct->id) }}"
                                        ...
                                        <tr class="calc-tr">
                                            ...
                                            <td>
                                                <input type="date" class="date py-2 my-input-disable-class bg-white editable-five d-none"
                                                    value="{{ @$wp->date && strtotime($wp->date) ? \Carbon\Carbon::parse($wp->date)->format('Y-m-d') : '' }}"
                                                    style="border:none">
                                                    <div class="input-group d-flex align-items-center">
                                                        <span
...
@endif
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const editor = new Jodit("#wo-desc", {
                buttons: "bold,italic,underline,|,fontsize,font,brush",
                style: {
                    fontFamily: 'CustomFont, Cambria, serif'
                },
                font: {
                    "Cambria": "CustomFont"
                }
            ...
            // Sync Jodit content to #scope_work
            editor.events.on('change', () => {
                document.getElementById('scope_work').innerHTML = editor.value;
            });
        });
    </script>
    <script>
