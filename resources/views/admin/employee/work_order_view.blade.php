...
<body>
    <div class="container">
        ...
        <table class="custom-table mt-4">
            ...
            <tbody>
                ...
                <tr>
                    <th><b>Scheduled date: </b> {{ $workOrder->open_date }}</th>
                </tr>
                <tr>
                    <th><b>Scheduled time: </b> {{ @$workOrder->h_operation }}</th>
                </tr>
                <tr>
                    <th><b>Location Name: </b> {{ @$workOrder->site->location }}
                    </th>
                </tr>
                <tr>
                    <th><b>Site ID: </b>{{ @$workOrder->site->site_id }}
                    </th>
                </tr>
                <tr>
                    <th><b>Address, City, State & Zip:</b> {{ @$workOrder->site->address_1 }},
                        {{ @$workOrder->site->address_2 }} {{ @$workOrder->site->city }}, {{ @$workOrder->site->state }} -
                        {{ @$workOrder->site->zipcode }}
                    </th>
                </tr>
                <tr>
                    <th><b>Site Contact: </b> {{ @$workOrder->site_contact_name }}</th>
                </tr>
                <tr>
                    <th><b>Main Telephone: </b> {{ @$workOrder->main_tel }}</th>
                </tr>
            </tbody>
        </table>
        <section class=" others-section mt-4">
            <div class="contact-info mt-4">
                <h5 style="font-weight: bold; color: black;">Project Manager: {{ @$workOrder->employee->name }}</h5>
                <h5 style="font-weight: bold; color: black;">Phone: {{ @$workOrder->employee->mobile }}</h5>
                <h5 style="font-weight: bold; color: black;">Email: {{ @$workOrder->employee->email }}</h5>
            </div>
            ...
            <div class="my-4">
                ...
                <h5 class="my-4">{!! @$workOrder->instruction !!}</h5>
                <h5>{{ @$workOrder->a_instruction }}</h5>
                <!-- <div>
                    @if (@$workOrder->e_checkin == null)
                    <h5>Earliest Check-in: N/A</h5>
                    @else
                    <h4>Earliest Check-in: {{ @$workOrder->e_checkin }}</h4>
                    @endif
                    @if (@$workOrder->l_checkin == null)
                    <h5>Latest Check-in: N/A</h5>
                    @else
                    <h5>Latest Check-in: {{ @$workOrder->l_checkin }}</h5>
                    @endif
                </div> -->
                <hr>
                <div class="">
            ...
            <div class="" style="text-align: center;">
                ...
                @endphp
                @if (@$imageData)
                <img src="data:image/jpeg;base64,{{ $imageData }}" alt="Random Image" style="border: 1px solid #555; width: 100%; height: 250px; margin: 2px;">
                @else
                <img src="https://t4.ftcdn.net/jpg/02/51/95/53/360_F_251955356_FAQH0U1y1TZw3ZcdPGybwUkH90a3VAhb.jpg" alt="Default Image" style="border: 1px solid #555;">
                @endif
                @endforeach
                @else
    ...
    <a href="{{ url('pdf/work/order/download/') }}/{{ $workOrder->id }}" class="download-btn w-25">Download PDF</a>
</body>