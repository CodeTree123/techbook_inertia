...
class DistanceMatrixController extends Controller
{
    ...
    public function findClosestLocations(Request $request)
    {
        ...
        foreach ($locations as $location) {
            ...
            $distance = $location->greatCircleDistance($fromLatitude, $fromLongitude, $givenLatitude, $givenLongitude);
        ...
        foreach ($data['rows'] as $index => $row) {
            if (isset($row['elements'][0]['status']) && $row['elements'][0]['status'] === "OK") {
                ...
                if ($ftech) {
                    ...
                    $rateString = collect($ftech->rate)->map(function ($value, $key) {
                        return "$key : $value";
                    })->join(', ');
        ...
        if (!$techniciansFound) {
            return response()->json(['errors' => 'No technicians found in 50 miles radius.'], 404);
        }
